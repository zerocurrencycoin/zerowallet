...
run_dotranslations
$QMAKE zero-qt-wallet.pro -spec linux-g++ CONFIG+=release
...
cp -f zerowallet          "$tgzdir/"
cp -f "$ZERO_DIR/zerod"    "$tgzdir/"
cp -f "$ZERO_DIR/zero-cli" "$tgzdir/"
cp -f README.md           "$tgzdir/"
[ -z "${SKIP_STRIP:-}" ] && strip "$tgzdir/zerowallet" "$tgzdir/zerod" "$tgzdir/zero-cli"

(cd bin/tgz && tar czf "linux-zerowallet-v${APP_VERSION}.tgz" "linux-zerowallet-v${APP_VERSION}/") || err 'tar failed'
