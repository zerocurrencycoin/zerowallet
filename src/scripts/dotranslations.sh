...
ls res/*.qm | awk -F '[_.]' '{print $4}' | while read -r language ; do 
    if [ -f $QT_STATIC/translations/qtbase_$language.qm ]; then
        $QT_STATIC/bin/lconvert -o res/safe_$language.qm $QT_STATIC/translations/qtbase_$language.qm res/zero-qt-wallet_$language.qm
        mv res/safe_$language.qm res/zero-qt-wallet_$language.qm
    fi
done
