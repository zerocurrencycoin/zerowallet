...
Remove-Item -Force -ErrorAction Ignore ./artifacts/linux-binaries-zero-qt-wallet-v$version.tar.gz
Remove-Item -Force -ErrorAction Ignore ./artifacts/linux-deb-zero-qt-wallet-v$version.deb
Remove-Item -Force -ErrorAction Ignore ./artifacts/Windows-binaries-zero-qt-wallet-v$version.zip
Remove-Item -Force -ErrorAction Ignore ./artifacts/Windows-installer-zero-qt-wallet-v$version.msi
Remove-Item -Force -ErrorAction Ignore ./artifacts/macOS-zero-qt-wallet-v$version.dmg
...
scp -r src/ res/ ./zerowallet.pro ./application.qrc ./LICENSE ./README.md ${server}:/tmp/zqwbuild/ | Out-Null
...
if (! (Test-Path ./artifacts/linux-binaries-zero-qt-wallet-v$version.tar.gz) -or
    ! (Test-Path ./artifacts/linux-deb-zero-qt-wallet-v$version.deb) -or
    ! (Test-Path ./artifacts/Windows-binaries-zero-qt-wallet-v$version.zip) -or
    ! (Test-Path ./artifacts/Windows-installer-zero-qt-wallet-v$version.msi) ) {
        Write-Host "[Error]"
        exit 1;
    }
Write-Host "[OK]"
