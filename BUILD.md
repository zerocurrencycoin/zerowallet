...
|-------|----------|----------|
| `__OPTIMIZE__ predefined macro was enabled in PCH file but is currently disabled` | macOS | PCH built with different CONFIG (debug vs release). qmake may not rebuild the `.pch` when only flags change. Run `./src/scripts/mkdev.sh -c -L` (clean removes `bin/ZeroWallet_*.pch`). |
